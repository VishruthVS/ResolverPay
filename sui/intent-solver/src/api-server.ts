...
async function main() {
  ...
  if (!USER_KEY) console.warn('⚠️  USER_PRIVATE_KEY not set in .env – /api/intent/create and /cancel will fail');
  if (!SOLVER_KEY) console.warn('⚠️  SOLVER_PRIVATE_KEY not set in .env – /api/intent/execute will fail');
  ...
  // ================================================================
  app.post('/api/quote', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const rawInputAmount = Math.round(humanAmount * Math.pow(10, inputDecimals)).toString();

      console.log(`📡 Quote request: ${humanAmount} ${from} -> ${to}  (raw ${rawInputAmount})`);

      const quote = await solver.getSwapQuote(inputType, outputType, rawInputAmount);

      // Convert raw output back to human-readable
      const outputHuman = Number(quote.outputAmount) / Math.pow(10, outputDecimals);
  ...
  //    { "base": "SUI", "quote": "USDC" }
  // ================================================================
  app.post('/api/orderbook', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const level2 = await solver.getLevel2Data(pool.poolId, pool.baseType, pool.quoteType);
  ...
  //    { "pair": "SUI_USDC" }
  // ================================================================
  app.post('/api/price', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { pair } = req.body;
    ...
    try {
      ...
      res.json({
        ...
        intent: {
          ...
          inputAmountHuman: rawToHuman(intent.input_amount, intent.input_type),
          minOutputHuman: rawToHuman(intent.min_output_amount, intent.output_type),
  ...
  // ================================================================
  app.post('/api/intent/create', async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!userKeypair || !userAddress) {
        res.status(500).json({ error: 'USER_PRIVATE_KEY not configured in .env' });
        return;
      }
      ...
      const deadlineMs = String(Date.now() + (deadlineSeconds || 300) * 1000);

      const keypair = userKeypair;
      const sender = userAddress;

      console.log(`📡 Create intent: ${amount} ${from} → ${to} (min ${minOutput}) by ${sender.slice(0, 10)}...`);
      ...
      if (inputType === SUI_TYPE) {
        ...
        const coins = await suiClient.getCoins({ owner: sender, coinType: inputType });
        if (!coins.data.length) {
          res.status(400).json({ error: `No ${from} coins found in wallet ${sender}` });
          return;
        }
  ...
  // ================================================================
  app.post('/api/intents/open', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (const r of results) {
        ...
        openIntents.push({
          ...
          deadlineISO: new Date(Number(intent.deadline)).toISOString(),
          timeRemainingMs,
          timeRemainingHuman: timeRemainingMs > 0
      ...
      // Sort: most recent (latest deadline) first
      openIntents.sort((a, b) => Number(b.deadline) - Number(a.deadline));
  ...
  // ================================================================
  app.post('/api/intent/execute', async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!solverKeypair || !solverAddress) {
        res.status(500).json({ error: 'SOLVER_PRIVATE_KEY not configured in .env' });
        return;
      }
      ...
      if (!intent) {
        res.status(404).json({ error: `Intent ${intentId} not found on-chain` });
        return;
      }
      if (intent.status !== STATUS_OPEN) {
        res.status(400).json({ error: `Intent is not open (status: ${STATUS_LABEL[intent.status]})` });
        return;
      }
      ...
      console.log(`📡 Execute intent ${intentId.slice(0, 10)}... by solver ${solverAddr.slice(0, 10)}...`);
      ...
      const outputCoins = await suiClient.getCoins({ owner: solverAddr, coinType: outputType });
      ...
      const totalOutputBalance = outputCoins.data.reduce((s, c) => s + BigInt(c.balance), 0n);
      ...
      if (outputCoins.data.length > 1) {
        tx.mergeCoins(sourceCoin, outputCoins.data.slice(1).map(c => tx.object(c.coinObjectId)));
      }
      const [paymentCoin] = tx.splitCoins(sourceCoin, [tx.pure.u64(outputToProvide.toString())]);
      ...
      const execEvent = result.events?.find(e => e.type.includes('IntentExecuted'));
      const execData = execEvent?.parsedJson as any;

      console.log(`✅ Intent executed: ${intentId.slice(0, 10)}... | tx: ${result.digest}`);
  ...
  //    { "intentId": "0x..." }
  // ================================================================
  app.post('/api/intent/cancel', async (req: Request, res: Response, next: NextFunction) => {
    try {
      if (!userKeypair || !userAddress) {
        res.status(500).json({ error: 'USER_PRIVATE_KEY not configured in .env' });
        return;
      ...
      console.log(`📡 Cancel intent ${intentId.slice(0, 10)}... by ${sender.slice(0, 10)}...`);
      ...
      console.log(`✅ Intent cancelled: ${intentId.slice(0, 10)}... | tx: ${result.digest}`);
  ...
  //    { "address": "0x..." }
  // ================================================================
  app.post('/api/wallet/balance', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      for (const [alias, coinType] of Object.entries(TOKEN_ALIASES)) {
        try {
          const bal = await suiClient.getBalance({ owner: address as string, coinType });
          balances[alias] = {
  ...
  //    { "limit": 20 }
  // ================================================================
  app.post('/api/intents/history', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      res.json({
        ...
        created: created.map(e => ({
          ...
          minOutputHuman: rawToHuman(e.min_output_amount, resolveToken('USDC')),
        })),
        executed: executed.map(e => ({
          ...e,
          inputAmountHuman: rawToHuman(e.input_amount, resolveToken('SUI')),
          outputAmountHuman: rawToHuman(e.output_amount, resolveToken('USDC')),
        })),
      });
    } catch (err) {
      next(err);
  ...
  // ================================================================
  app.post('/api/config', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const configId = req.body?.configId || solverConfig.protocolConfigId;

      if (configId === '0x0') {
        res.status(400).json({
          error: 'PROTOCOL_CONFIG_ID not set in environment and not provided in body',
  ...
  // ================================================================
  app.post('/api/intent/build/create', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { sender, from, to, amount, minOutput, deadlineSeconds } = req.body;

      if (!sender || !from || !to || amount === undefined || minOutput === undefined) {
        res.status(400).json({
          error: 'Missing required fields: sender, from, to, amount, minOutput',
          example: {
      ...
      console.log(`📡 [build] Create intent: ${amount} ${from} → ${to} for ${(sender as string).slice(0, 10)}...`);
      ...
      if (inputType === SUI_TYPE) {
        ...
        const coins = await suiClient.getCoins({ owner: sender as string, coinType: inputType });
        if (!coins.data.length) {
          res.status(400).json({ error: `No ${from} coins found in wallet ${sender}` });
          return;
        }
      ...
      res.json({
        success: true,
        txBytes,
        message: 'Sign this transaction with your wallet, then submit via /api/tx/execute or directly to Sui RPC.',
        details: {
          sender,
          inputToken: from,
          outputToken: to,
  ...
  // ================================================================
  app.post('/api/intent/build/execute', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      console.log(`📡 [build] Execute intent ${(intentId as string).slice(0, 10)}... for solver ${solverAddr.slice(0, 10)}...`);
      ...
      res.json({
        ...
        details: {
          ...
          outputToProvideHuman: rawToHuman(outputToProvide.toString(), outputType),
          inputType,
          outputType,
        },
      });
  ...
  //  Returns: { txBytes: "<base64>" }
  // ================================================================
  app.post('/api/intent/build/cancel', async (req: Request, res: Response, next: NextFunction) => {
    try {
      const { sender, intentId } = req.body;
      ...
      console.log(`📡 [build] Cancel intent ${(intentId as string).slice(0, 10)}... for ${(sender as string).slice(0, 10)}...`);
  ...
  // ================================================================
  app.post('/api/tx/execute', async (req: Request, res: Response, next: NextFunction) => {
    try {
      ...
      const intentCreated = result.events?.find(e => e.type.includes('IntentCreated'));
      const intentExecuted = result.events?.find(e => e.type.includes('IntentExecuted'));
      const intentCancelled = result.events?.find(e => e.type.includes('IntentCancelled'));
  ...
  app.listen(PORT, () => {
    ...
    console.log(`   POST /api/intent/create   { from, to, amount, minOutput, deadlineSeconds }  ← USER_PRIVATE_KEY`);
    console.log(`   POST /api/intents/open    { limit? }`);
    console.log(`   POST /api/intent          { id }`);
    console.log(`   POST /api/intent/execute  { intentId }                                      ← SOLVER_PRIVATE_KEY`);
    console.log(`   POST /api/intent/cancel   { intentId }                                      ← USER_PRIVATE_KEY`);
    console.log(`   POST /api/intents/history { limit? }`);
    console.log('');
    console.log('   ── 🔐 Wallet-Safe (frontend integration) ────────────');
    console.log(`   POST /api/intent/build/create   { sender, from, to, amount, minOutput, deadlineSeconds }`);
    ...
    console.log(`   curl -X POST http://localhost:${PORT}/api/intent/build/create \\`);
    console.log(`     -H "Content-Type: application/json" \\`);
    console.log(`     -d '{"sender":"0xYourAddress","from":"SUI","to":"USDC","amount":0.01,"minOutput":0.009,"deadlineSeconds":300}'`);
    console.log('='.repeat(60));
    console.log('');
  });
}
